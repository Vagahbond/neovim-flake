{lib, ...}: let
  inherit (lib.options) mkOption mkEnableOption;
  inherit (lib.types) int bool str listOf enum submodule oneOf either nullOr;

  lualineComponentName = enum [
    "branch"
    "buffers"
    "diagnostis"
    "diff"
    "encoding"
    "fileformat"
    "filename"
    "filesize"
    "filetype"
    "hostname"
    "location"
    "mode"
    "progress"
    "searchcount"
    "selectioncount"
    "tabs"
    "windows"
    ""
  ];

  lualineBaseComponentOptions = {
    name = mkOption {
      type = enum lualineComponentName;
      description = "The name of the component";
      default = "";
    };
    iconsEnabled = mkOption {
      type = bool;
      description = "Whether to draw component even if it's empty. Might be useful if you want just the separator.";
      default = true;
    };
    icon = mkOption {
      description = ''
        Defines the icon to be displayed in front of the component.
        Can be string|table
        As table it must contain the icon as first entry and can use
        color option to custom color the icon. Example:
        {'branch', icon = ''} / {'branch', icon = {'', color={fg='green'}}}
        icon position can also be set to the right side from table. Example:
        {'branch', icon = {'', align='right', color={fg='green'}}}
      '';
      type = str;
      default = null;
    };
    separator = mkOption {
      description = ''
        Determines what separator to use for the component.
        Note:
          When a string is provided it's treated as component_separator.
          When a table is provided it's treated as section_separator.
          Passing an empty string disables the separator.

          These options can be used to set colored separators
          around a component.

        The options need to be set as such:
          separator = { left = '', right = ''}

        Where left will be placed on left side of component,
        and right will be placed on its right.
      '';
      example = {
        left = "";
        right = "";
      };
      type = either str (submodule {
        options = {
          left = mkOption {
            type = nullOr str;
            default = null;
          };
          right = mkOption {
            type = nullOr str;
            default = null;
          };
        };
      });
    };
    cond = mkOption {
      type = nullOr str;
      description = "Condition function, the component is loaded when the function returns `true`.";
      default = null;
      example = ""; # TODO: find an example
    };
    drawEmpty = mkOption {
      type = bool;
      description = "Whether to draw component even if it's empty. Might be useful if you want just the separator.";
      default = false;
    };
    # TODO: deep-nix that option
    color = mkOption {
      description = ''
         Defines a custom color for the component:

         'highlight_group_name' | { fg = '#rrggbb'|cterm_value(0-255)|'color_name(red)', bg= '#rrggbb', gui='style' } | function
         Note:
          '|' is synonymous with 'or', meaning a different acceptable format for that placeholder.
         color function has to return one of other color types ('highlight_group_name' | { fg = '#rrggbb'|cterm_value(0-255)|'color_name(red)', bg= '#rrggbb', gui='style' })
         color functions can be used to have different colors based on state as shown below.

         Examples:
           color = { fg = '#ffaa88', bg = 'grey', gui='italic,bold' },
           color = { fg = 204 }   -- When fg/bg are omitted, they default to the your theme's fg/bg.
           color = 'WarningMsg'   -- Highlight groups can also be used.
           color = function(section)
              return { fg = vim.bo.modified and '#aa3355' or '#33aa88' }
           end,
        The default is your theme's color for that section and mode.
      '';
      default = null;
      type = nullOr str;
      example = "{ fg = '#ffaa88', bg = 'grey', gui='italic,bold' }";
    };
    type = mkOption {
      description = ''
        Specify what type a component is, if omitted, lualine will guess it for you.

        Available types are:
          [format: type_name(example)], mod(branch/filename),
          stl(%f/%m), var(g:coc_status/bo:modifiable),
          lua_expr(lua expressions), vim_fun(viml function name)

        Note:
        lua_expr is short for lua-expression and vim_fun is short for vim-function.
      '';
      type = nullOr str;
      default = null;
    };
    padding = mkOption {
      description = ''
        Adds padding to the left and right of components.
        Padding can be specified to left or right independently, e.g.:
        padding = { left = left_padding, right = right_padding }
      '';
      type = either int (submodule {
        options = {
          left = mkOption {
            type = nullOr str;
            default = null;
          };
          right = mkOption {
            type = nullOr str;
            default = null;
          };
        };
      });
      default = 1;
      example = {
        left = 0;
        right = 1;
      };
    };
    fmt = mkOption {
      description = ''
        Format function, formats the component's output.
        This function receives two arguments:
        - string that is going to be displayed and
          that can be changed, enhanced and etc.
        - context object with information you might
          need. E.g. tabnr if used with tabs.
      '';
      type = nullOr str;
      example = ""; #TODO: Find an example
      default = null;
    };
    onClick = mkOption {
      description = ''
        takes a function that is called when component is clicked with mouse.
        the function receives several arguments
        - number of clicks in case of multiple clicks
        - mouse button used (l(left)/r(right)/m(middle)/...)
        - modifiers pressed (s(shift)/c(ctrl)/a(alt)/m(meta)...)
      '';
      default = null;
      type = nullOr str;
      example = ""; #TODO: Find an example
    };
  };

  lualineBufferComponentOptiions = {
  };

  lualineDatetimeComponentOptiions = {
  };

  lualineDiagnosticsComponentOptiions = {
  };

  lualineDiffComponentOptiions = {
  };

  lualineFileFormatComponentOptiions = {
  };

  lualineFileNameComponentOptiions = {
  };

  lualineFileTypeComponentOptiions = {
  };

  lualineSearchCountComponentOptiions = {
  };

  lualineTabsComponentOptiions = {
  };

  lualineWindowsComponentOptiions = {
  };

  lualineBaseComponent = enum submodule {
    options = lualineBaseComponentOptions;
  };

  lualineElement = oneOf [lualineBaseComponent];
in
  {
    description,
    default,
  }:
    mkOption {
      inherit description default;
      type = listOf lualineElement;
      example = [
        {
          content = "mode";
          iconsEnabled = true;
          separator = {
            left = "▎";
            right = "";
          };
        }
        {
          drawEmpty = true;
          separator = {
            left = "";
            right = "";
          };
        }
      ];
    }
